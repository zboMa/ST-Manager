...
export default function cardGrid() {
  return {
    ...
    init() {
      ...
      this.$watch("$store.global.viewState.searchType", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("type");
      });
      this.$watch("$store.global.viewState.searchScope", () => {
        this.currentPage = 1;
      ...
      this.$watch("$store.global.viewState.filterTags", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.excludedTags", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      // 监听排除列表变化，触发刷新
      ...
      this.$watch("$store.global.viewState.importDateFrom", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.importDateTo", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.modifiedDateFrom", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.modifiedDateTo", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.tokenMin", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      this.$watch("$store.global.viewState.tokenMax", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });

      // 监听当前会话排序变化（不写入配置）
      this.$watch("$store.global.currentSort", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      ...
      // 监听收藏过滤变化
      this.$watch("$store.global.viewState.favFilter", () => {
        this.currentPage = 1;
        this.scheduleFetchCards("filters");
      });
      // 监听设置中的收藏前置变化
      ...
      window.addEventListener("get-all-card-ids", () => {
        ...
        window.dispatchEvent(
          new CustomEvent("all-card-ids-response", {
            detail: { ids: cardIds },
          }),
        );
      });
      ...
      window.addEventListener("card-updated", (e) => {
        ...
        if (idx === -1 && updatedCard.is_bundle && updatedCard.bundle_dir) {
          idx = this.cards.findIndex(
            (c) => c.is_bundle && c.bundle_dir === updatedCard.bundle_dir,
          );
        }
        ...
        cards.forEach((card) => {
          ...
          // 2. 如果当前在子目录视图
            ...
            if (card.category === currentViewCat) {
              ...
              isRecursive &&
              card.category.startsWith(currentViewCat + "/")
            ) {
    ...
    cardHasLocalNote(card) {
      const summary =
        card && typeof card.ui_summary === "string"
          ? card.ui_summary.trim()
          : "";
    ...
    getReadableTagChipStyle(tag) {
      ...
      const enhanced =
        "--tag-chip-backdrop:blur(7px);--tag-chip-shadow:inset 0 1px 0 rgba(255,255,255,0.08),0 1px 2px rgba(0,0,0,0.35);";
    ...
    get visibleFlippedCount() {
      return this.paginatedCards.reduce(
        (count, card) => count + (this.isCardFlipped(card.id) ? 1 : 0),
        0,
      );
    },

    toggleBulkFlipMode() {
      if (this.bulkBackMode) {
    ...
    flipAllCardsToBack() {
      ...
      this.paginatedCards.forEach((card) => {
        next[String(card.id)] = true;
      });

      this.flippedCardIds = next;
      this.bulkBackMode = true;
    },
    ...
    scheduleAutoFlipBack(cardId) {
      ...
      if (this.bulkBackMode) return;
      if (!this.isCardFlipped(cardId)) return;

      const key = String(cardId);
      this.clearAutoFlipBackTimer(key);

      this._autoFlipBackTimers[key] = setTimeout(() => {
        if (this.bulkBackMode || !this.isCardFlipped(cardId)) {
          this.clearAutoFlipBackTimer(key);
          return;
        }
        ...
        this.flippedCardIds = next;
        this.clearAutoFlipBackTimer(key);
      }, this.autoFlipBackDelayMs);
    },

    clearAutoFlipBackTimer(cardKey) {
      const timer = this._autoFlipBackTimers[cardKey];
      if (!timer) return;

      clearTimeout(timer);
      delete this._autoFlipBackTimers[cardKey];
    },

    clearAllAutoFlipBackTimers() {
      Object.keys(this._autoFlipBackTimers).forEach((key) =>
        this.clearAutoFlipBackTimer(key),
      );
    },
    ...
    fetchCards() {
      ...
      // 取消上一次未完成请求
      try {
        if (this._fetchCardsAbort) this._fetchCardsAbort.abort();
      } catch (e) {
        console.error(e);
      }
      ...
      const params = {
        ...
        import_date_from: vs.importDateFrom || "",
        import_date_to: vs.importDateTo || "",
        modified_date_from: vs.modifiedDateFrom || "",
        modified_date_to: vs.modifiedDateTo || "",
      ...
      listCards(params) // 调用 API 模块
        .then((data) => {
          ...
          if (Array.isArray(data.sidebar_tag_groups)) {
            store.sidebarTagGroups = data.sidebar_tag_groups;
          }
    ...
    changePage(p) {
      if (p >= 1 && p <= this.totalPages) {
        ...
        window.dispatchEvent(
          new CustomEvent("card-page-changed", { detail: { page: p } }),
        );
        this.fetchCards();
      }
    },
    ...
    dragStart(e, card) {
      ...
      if (cardElement) {
        ...
        if (e.dataTransfer.setDragImage) {
          ...
          const displayCard =
            this.draggedCards.length > 1
              ? this.cards.find((c) => c.id === this.draggedCards[0])
              : card;
    ...
    _uploadFilesInternal(files, targetCategory) {
      ...
      if (targetCategory === null || targetCategory === undefined) {
        ...
        targetCategory =
          currentCat === "" || currentCat === "根目录" ? "" : currentCat;
      }
      ...
      fetch("/api/upload/stage", {
        ...
        .then((res) => {
          ...
          if (res.success) {
            ...
            const validReport = res.report.filter(
            ...
            if (
              validReport.length === 1 &&
              errors.length === 0 &&
              validReport[0].status === "ok"
            ) {
              ...
              fetch("/api/upload/commit", {
                ...
                .then((cRes) => {
                  ...
                  if (cRes.success) {
                    ...
                    window.dispatchEvent(
                      new CustomEvent("batch-cards-imported", {
            ...
            // 打开批量导入确认弹窗 (多文件或有冲突时)
            window.dispatchEvent(
              new CustomEvent("open-batch-import-modal", {
                detail: {
    ...
    dropCards(targetCat) {
      ...
      if (
        !confirm(
          `移动 ${this.draggedCards.length} 张卡片到 "${targetCatName}"?`,
        )
      ) {
        this.draggedCards = [];
        return;
    ...
    insertCardSorted(newCard) {
      const sortMode =
        this.$store.global.currentSort ||
        this.$store.global.settingsForm.default_sort ||
        "date_desc";
      let index = -1;
      ...
      const compare = (a, b) => {
        ...
        if (sortMode === "import_desc")
          return (
            (b.import_time || b.last_modified || 0) -
            (a.import_time || a.last_modified || 0)
          );
        if (sortMode === "import_asc")
          return (
            (a.import_time || a.last_modified || 0) -
            (b.import_time || b.last_modified || 0)
          );
        if (sortMode === "name_asc")
          return String(a.char_name).localeCompare(
            String(b.char_name),
            "zh-CN",
          );
        if (sortMode === "name_desc")
          return String(b.char_name).localeCompare(
            String(a.char_name),
            "zh-CN",
          );
        if (sortMode === "token_desc")
          return (b.token_count || 0) - (a.token_count || 0);
        if (sortMode === "token_asc")
          ...
          if (res.success) {
            ...
            if (shouldOpenDetail) {
              setTimeout(() => {
                ...
                if (foundCard) {
                  window.dispatchEvent(
                    new CustomEvent("open-detail", { detail: foundCard }),
                  );
                }
              }, 500);
            }
