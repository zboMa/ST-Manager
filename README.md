...
ST-Manager 是一款专为 SillyTavern AI 聊天程序设计的资源可视化管理工具。它提供了一个现代化的 Web 界面，帮助用户高效管理角色卡、聊天记录、世界书、扩展脚本等各种资源，支持批量操作、自动化规则引擎、智能缓存等功能。

### ✨ 核心特性

- 🎴 **角色卡管理** - 支持 PNG/JSON 格式角色卡的浏览、编辑、导入导出
- 💬 **聊天记录管理** - 支持 SillyTavern `.jsonl` 聊天导入、角色绑定、全文检索、沉浸式阅读与楼层编辑
- 🖥️ **聊天实例舞台** - 阅读器支持整页实例模式，可直接运行楼层中的前端片段并切换实例楼层
...
- 🛠️ **脚本运行调试** - ST 脚本支持运行、重载、停止、日志查看与按钮触发
- ⚡ **快速回复管理** - 快速回复模板管理，支持分类、搜索和批量操作
- 🤖 **自动化引擎** - 基于规则的自动化任务执行，支持复杂的条件判断
- 🏷️ **论坛标签抓取** - 支持从Discord论坛（类脑）自动抓取帖子标签并应用到角色卡
- 🕒 **导入时间追踪** - 记录角色卡首次导入时间，支持导入时间排序与展示
- 🚀 **发送到 ST** - 角色卡网格支持一键发送到 SillyTavern，并记录最近发送时间
- 🔄 **实时同步** - 文件系统自动监听，实时同步变更到数据库
- 🎨 **可视化界面** - 现代化响应式 UI，支持暗色/亮色主题
- 🪄 **小图翻页交互** - 支持卡片正反面查看、一键翻页与自动翻回
- 📦 **版本管理** - 支持角色卡 Bundle 多版本管理
- 🏷️ **标签系统** - 支持标签分类、颜色/透明度、自定义筛选与批量管理
- 🔍 **智能搜索** - 支持名称、文件名、标签、创作者等多维度搜索
- 🧭 **搜索范围控制** - 支持当前目录 / 全部目录 / 全量搜索模式
- 🧪 **运行时检查器** - 统一查看 render/script 运行状态，并支持脚本定位与控制
- 🔗 **酒馆资源同步** - 从本地 SillyTavern 读取并同步角色卡、聊天记录、世界书、预设、正则、ST脚本、快速回复
...
- 已整理从 `9e6a4cd`（不含）到最新版本的面向用户更新说明：[`docs/changelog-9e6a4cd-to-latest.md`](docs/changelog-9e6a4cd-to-latest.md)
- 本轮重点包括：聊天阅读器整页实例模式、长聊天分层渲染与窗口化加载、ST 脚本运行调试、Runtime Inspector、角色卡一键发送到 ST
...
│   │   └── st_client.py      # SillyTavern 本地资源客户端
│   │
│   ├── automation/            # 自动化引擎
│   │   ├── engine.py         # 规则引擎核心
│   │   ├── manager.py        # 规则集管理
...
│   │   ├── db_session.py     # 数据库会话
│   │   ├── cache.py          # 全局缓存
│   │   ├── chat_store.py     # 聊天索引与元数据存储
│   │   └── ui_store.py       # UI 数据存储
│   │
│   └── utils/                 # 工具函数
│       ├── data.py           # 数据处理工具
│       ├── chat_parser.py    # 聊天记录解析工具
...
#### 说明
- `default_sort`：角色卡默认排序方式，支持 `date_desc/date_asc/import_desc/import_asc/name_asc/name_desc/token_desc/token_asc`
...
| **收藏标记** | 快速收藏常用角色 |
| **搜索过滤** | 多维度搜索和标签过滤 |
| **搜索范围** | 支持“当前目录 / 全部目录 / 全量”三种搜索范围 |
| **小图翻页交互** | 卡片支持正反面翻页、当前页一键翻页、桌面端自动翻回 |
| **标签分类体系** | 标签支持分类管理、颜色与透明度设置、分组显示与分类筛选 |
| **时间信息** | 区分“导入时间 / 修改时间 / 上次发送到 ST”，支持在网格背面和详情页查看 |
| **发送到 ST** | 卡片网格支持一键发送到 SillyTavern，并在发送中/已发送时显示状态 |
| **Bundle 管理** | 支持多版本角色聚合显示 |

#### 目录删除行为说明
- 右键删除文件夹时，默认“未勾选”会执行“文件夹解散”：将内部内容移动到上一级目录，最后删除该空目录。
- 勾选“删除子文件/子文件夹（递归删除）”后，会递归删除该文件夹及其子内容，并移动到回收站（可恢复），同时清理索引与本地 UI 数据。
...
<a id="nav-feature-chats"></a>

### 聊天记录管理

ST-Manager 现支持将 SillyTavern `.jsonl` 聊天记录作为独立资源进行统一管理，并可与角色卡建立双向关联。

#### 核心功能

| 功能 | 描述 |
|------|------|
| **聊天网格** | 独立聊天模式，支持分页、收藏、按绑定状态筛选 |
| **导入与同步** | 支持拖拽导入 `.jsonl`，也支持从本地 SillyTavern 一键同步 |
| **角色绑定** | 可把聊天绑定到角色卡，并从角色卡详情页直接反查 |
| **沉浸式阅读** | 全屏三栏阅读器，支持楼层导航、全文搜索、收藏楼层与实例楼层直达 |
| **本地元数据** | 支持显示名称、备注、最近阅读楼层、收藏状态等本地索引信息 |
| **楼层编辑** | 支持单层消息编辑、批量查找替换并写回原始聊天文件 |
| **解析规则** | 支持本地 / 角色卡 / 聊天专属显示规则链，兼容 ST Regex / 预设导入与整条消息预览 |
| **整页实例** | 支持识别可执行楼层，在阅读器中切换到整页实例模式运行与浏览 |
| **长聊天优化** | 支持分层渲染、历史楼层折叠与窗口化加载，减少大聊天阅读压力 |

#### 支持的聊天能力

- 支持 `.jsonl` 首行元数据 + 消息体解析
- 可提取正文、思维链、小总结、选项、时间栏等结构化内容
- 收藏楼层与聊天备注等附加信息写入本地索引，不污染角色卡数据
- 可识别部分整页 HTML / 前端片段楼层，并在阅读器中以实例模式单独查看
...
| **运行调试** | 可直接运行 / 重载 / 停止脚本，查看隔离预览、日志与按钮触发结果 |
| **运行时排查** | 可配合 Runtime Inspector 查看脚本状态、快照并快速定位到对应脚本 |
...
用于把多个含义接近的标签统一到一个目标标签，便于后续筛选与统计。
...
{
  "type": "merge_tags",
  "value": "femdom→女上|female→女性|waifu→老婆"
}
```

说明：
- 每条映射使用 `旧标签→新标签`
- 多条规则用 `|` 分隔（当开启“斜杠作为标签分隔符”时，也支持 `/`）
- 一条规则可包含多个来源标签：`female|girl→女性`
- 在自动化“手动执行规则集”时，`merge_tags` 会参与执行（不再仅限手动打标流程）
...
```json
{
  ...
  "config": {
    "exclude_tags": ["其他", "待审核"],
    "replace_rules": {"其他": "杂项", "调教": "BDSM"},
    "merge_mode": "merge"
  }
}
```

2. **多值语法支持**：在界面配置时支持使用管道符 `|` 分隔多个值
   - 排除标签: `其他|待审核`
   - 标签替换: `其他→杂项|调教→BDSM`
   - `replace_rules` 支持对象与文本两种写法（如：`标签A→标签B|标签C->标签D`）

3. 确保角色卡的"来源链接"指向Discord论坛帖子

4. 配置Discord认证（见下方"Discord论坛认证配置"）

**触发时机**：
- **导入时**：全局规则集自动执行普通动作（移动文件夹、添加/移除标签、设置收藏、名称同步等），不执行 `fetch_forum_tags` 与 `merge_tags`
- **更新链接时**：在卡片详情页修改"来源链接"并保存时，触发 `fetch_forum_tags`，并在抓取后联动执行 `merge_tags`（若配置）
- **手动打标时**：仅在“批量标签管理输入框添加标签”和“详情页自定义添加标签输入框添加”时触发 `merge_tags`（从标签库点选不触发）
...
| **聊天记录** | chats | SillyTavern `data/default-user/chats/` | `data/library/chats/` |
...
`sort` 支持：`date_desc`、`date_asc`、`import_desc`、`import_asc`、`name_asc`、`name_desc`、`token_desc`、`token_asc`

`search_scope` 支持：`current`（当前目录）、`all_dirs`（全部目录）、`full`（全量，忽略分类/标签/收藏筛选，仅保留关键词搜索）
